...
`.wander <radius>` - make bots walk continuously in a Star of David (hexagram) pattern centered on their current position. The radius specifies the distance (in blocks) from the center to the outer tips of the star. While wandering, bots also perform swing attacks, interact actions, use items, and occasionally send random commands to increase packet activity.  
